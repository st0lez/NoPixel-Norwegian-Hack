...
async function start(duration,puzzlelength,amount){
    ...
    // mock loading screen
    setInformationText('Etablerer tilkobling')
    await delay(0.8)
    setInformationText('Masse tall på skjermen...')
    await delay(1)
    setInformationText('En feil har skjedd; Trenger menneskelige inputs..')
    await delay(0.8)
    ...
    // display result
    setInformationText((result) ? 'Systemet har blit åpnet.' : "Systemet likte ikke svarene dine")
    ...
    $('#submitted-reveal').textContent = (result) ? 'Good Job, indeed the' : ((submitted == null) ?  "Tiden rant ut," : `Du skrev "${submitted || ' '}", det`)
}
