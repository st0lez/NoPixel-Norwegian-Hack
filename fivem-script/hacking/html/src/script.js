...
async function start(){
    ...
    // mock loading screen
    setInformationText('ETABLISHING CONNECTION')
    await forsinkelse (0,8)
    setInformationText('GJØR HACKERMANS TING...')
    await forsinkelse (1)
    setInformationText('TILGANGSKODE FLAGGET; KREVER HUMAN CAPTCHA INNPUT..')
    await forsinkelse (0,8)
    ...
    // display result
    setInformationText((result) ? "systemet har blitt forbigått." : "Systemet godtok ikke svarene dine")
    ...
    $('#submitted-reveal').textContent = (result) ?'God jobb, faktisk' : ((innsendt == null) ? "Tiden rant ut," : `Du skrev "${submitted || ' '}", the`)
}
